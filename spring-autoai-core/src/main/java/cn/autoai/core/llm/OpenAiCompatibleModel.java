 ...
 */
public class OpenAiCompatibleModel implements AutoAiModel {
    ...
    public OpenAiCompatibleModel(String name, String baseUrl, String apiKey, ObjectMapper objectMapper) {
        this(name, baseUrl, apiKey, objectMapper, HttpClient.newBuilder()
            .connectTimeout(Duration.ofSeconds(30))
            .build());
    }

    public OpenAiCompatibleModel(String name, String baseUrl, String apiKey, ObjectMapper objectMapper, HttpClient httpClient) {
        this.name = name;
    ...
    public ChatCompletionResponse chat(ChatCompletionRequest request) {
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new IllegalStateException("OpenAI compatible API error: " + response.statusCode() + " " + response.body());
            }
            return objectMapper.readValue(response.body(), ChatCompletionResponse.class);
        } catch (Exception ex) {
            throw new IllegalStateException("OpenAI compatible model call failed", ex);
        }
    }
    ...
    public ChatCompletionResponse chatStream(ChatCompletionRequest request, StreamCallback callback) {
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                ...
                throw new IllegalStateException("OpenAI compatible API error: " + response.statusCode() + " " + errorBody);
            }
            return readStreamResponse(response.body(), request.getModel(), callback);
        } catch (Exception ex) {
            throw new IllegalStateException("OpenAI compatible stream call failed", ex);
        }
    }
    ...
    /**
     * Parse streaming output and concatenate into standard response.
     */
    private ChatCompletionResponse readStreamResponse(InputStream inputStream, String modelName, StreamCallback callback) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
            ...
            throw new IllegalStateException("OpenAI compatible stream parse failed", ex);
        }
